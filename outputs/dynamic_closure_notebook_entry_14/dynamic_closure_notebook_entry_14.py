...
OUT = Path("/mnt/data/dynamic_closure_entry_14_output")
...
def z3_character_projectors():
    """
    R is the regular shift on C^3.
    Pi_s = (1/3) sum_n omega^{-s n} R^n.
    """
    r = shift(3)
    ...
    for s, pi_s in enumerate(projectors):
        idem_err = float(np.max(np.abs(pi_s @ pi_s - pi_s)))
        herm_err = float(np.max(np.abs(pi_s.conj().T - pi_s)))
        rank = int(np.linalg.matrix_rank(pi_s, tol=TOL))
        rows.append({
            "sector_s": s,
            "idempotence_max_error": idem_err,
            "hermiticity_max_error": herm_err,
            "rank": rank,
        })
    ...
    for s in range(3):
        for t in range(3):
            ...
            orth_errors.append(
                np.max(np.abs(projectors[s] @ projectors[t] - target))
            )

    sum_err = float(np.max(np.abs(sum(projectors) - identity)))
    max_orth_err = float(max(orth_errors))

    summary = pd.DataFrame([{
        "projector_sum_identity_error": sum_err,
...
def triplication_isometry_certificate():
    """
    V|psi> = (1/sqrt(3)) sum_a |a> tensor U_a |psi>.
    Branch labels are orthogonal; if U_a are unitary, V is an isometry.
    """
    rng = np.random.default_rng(1401)
    dim = 4

    # random normalized state in H4
...
def braid_certificate():
    """
    Track strand labels through beta=(sigma1 sigma2)^6.
    Each positive crossing contributes +1 to the signed crossing count of
    the pair of closed components. Pairwise linking = half signed crossings.
    """
    positions = [1, 2, 3]
...
def uv_power_counting_certificate():
    """
    Model radial tail:
      I(Rmax) = ∫_1^Rmax r^(D-1+sigma-n*alpha) dr
    The infinite tail converges iff n*alpha > D + sigma.
    """
    cases = [
    ...
    for case in cases:
        ...
        for rmax in rmax_values:
            if abs(q) < 1e-14:
                integral = math.log(rmax)
            else:
                integral = (rmax**q - 1.0) / q
            vals.append(integral)

        rows.append({
            **case,
            "radial_exponent": exponent,
            "criterion_nalpha_gt_DplusSigma":
                case["n"] * case["alpha"] > case["D"] + case["sigma"],
            "I_R10": vals[0],
            "I_R100": vals[1],
            "I_R1000": vals[2],
            "I_R10000": vals[3],
...
def main():
    ...
    print()
    print("UV power-counting examples:")
