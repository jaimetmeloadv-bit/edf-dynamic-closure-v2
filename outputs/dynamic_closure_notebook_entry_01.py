...
class Config:
    ...
    # Ornstein-Uhlenbeck environmental fluctuation:
    ...
    # "null" is the only scientifically justified mode at this stage.
    score_mode: str = "null"
...
def closure_score_null(k_values: np.ndarray) -> np.ndarray:
    """
    Unbiased control:
        B_k = 0 for every closure sector.
    """
...
def closure_score_edf(k_values: np.ndarray) -> np.ndarray:
    """
    Placeholder for the exact EDF Golden/Fibonacci closure functional.

    Do not replace this with an arbitrary function centered at k = 12.
    The function must come from an independently derived EDF structural rule.
    """
...
def transition_matrix_from_score(
    ...
    """
    Build pairwise transition rates W_{j->k}.

    Convention:
    lower B_k means a more favorable structural sector.

    Symmetric detailed-balance form:
...
def simulate_one(
    ...
    # Start with equal unnormalized populations.
    ...
    for step, _ in enumerate(times):
        ...
        # Incoming flow for each destination k:
        incoming = q @ W

        # Outgoing flow from each source j:
        outgoing = q * W.sum(axis=1)

        dq = incoming - outgoing - gamma_eff * q
        q = q + config.dt * dq

        # Numerical positivity guard.
        q = np.maximum(q, 0.0)
...
def run_ensemble(config: Config) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    return summary, runs, example


def save_example_trajectory(
    example: dict,
    output_dir: Path,
) -> None:
    times = example["times"]
    k_values = example["k_values"]
    p_history = example["p_history"]

    trajectory = pd.DataFrame(
        p_history,
        columns=[f"k_{k}" for k in k_values],
    )
    trajectory.insert(0, "time", times)
    trajectory.to_csv(
        output_dir / "example_trajectory.csv",
        index=False,
    )
    ...
    entropy_frame.to_csv(
        output_dir / "example_entropy.csv",
        index=False,
    )
...
def make_plots(
    ...
    # Plot 1: ensemble selection probability
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "selection_probability.png",
        dpi=180,
    )
    plt.close()

    # Plot 2: mean residence
    plt.figure(figsize=(9, 5))
    plt.plot(
        summary["k"],
        summary["mean_residence"],
        marker="o",
    )
    plt.xlabel("Closure sector k")
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "mean_residence.png",
        dpi=180,
    )
    plt.close()

    # Plot 3: one realization
    plt.figure(figsize=(10, 6))
    for index, k in enumerate(example["k_values"]):
        plt.plot(
            example["times"],
            example["p_history"][:, index],
            linewidth=0.9,
            label=f"k={k}",
        )
    plt.xlabel("Time")
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "example_trajectory.png",
        dpi=180,
    )
    plt.close()

    # Plot 4: closure-space entropy
    plt.figure(figsize=(9, 5))
    plt.plot(
        example["times"],
        example["entropy_history"],
    )
    plt.xlabel("Time")
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / "closure_entropy.png",
        dpi=180,
    )
    plt.close()
...
def main() -> None:
    ...
    runs.to_csv(
        output_dir / "ensemble_runs.csv",
        index=False,
    )

    save_example_trajectory(
        example,
        output_dir,
    )
    ...
    print()
    print("Dynamic Closure Notebook — Entry 01")
