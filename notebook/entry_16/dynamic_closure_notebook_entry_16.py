...
OUT = Path("dynamic_closure_entry_16_output")
...
def main():
    ...
    counters.to_csv(OUT / "premise_countermodels.csv", index=False)
    k12 = k12_table()
    k12.to_csv(OUT / "k12_step_algebra_classification.csv", index=False)
    summary = pd.DataFrame([{
        "nonprimitive_formula_all_verified": bool(nonprim["formula_verified"].all()),
        "simple_spectrum_full_rank_all_verified": bool(simple["full_matrix_algebra_rank"].all()),
        "nonmultiplicative_span_formula_all_verified": bool(
            np.all(
